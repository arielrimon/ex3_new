...
struct ThreadContext {
    ...
    ThreadContext(JobContext *job_context, const int thread_id) {
        this->thread_id = thread_id;
        this->job_context = job_context;
        this->intermediate_vec = new IntermediateVec();
        job_context->all_intermediate_vec->push_back(this->intermediate_vec);
    }
};
...
void *map_reduce_method(void *context, JobContext *job_context) {
    ...
    while (current_index < job_context->input_vec->size()) {
        //Todo: how should I increase the percentage
        auto pair_to_map = job_context->input_vec->at(current_index);
        job_context->client->map(pair_to_map.first, pair_to_map.second, context);
    ...
    if (tc->thread_id == 0) {
        ...
        while (!all_intermediate_vec->at(0)->empty()) {
            ...
            pop_all_max_keys(max_key, max_key_vector, all_intermediate_vec, tc, 0);
    ...
    while (current_index < job_context->shuffled_intermediate_vec->size()) {
        auto vec_to_reduce = job_context->shuffled_intermediate_vec->at(current_index);
        job_context->client->reduce(vec_to_reduce, context);
        current_index = (*(job_context->reduce_atomic_counter))++;
    }
 ...
 * @param thread_id index in threads
 */
void init_thread_context(ThreadContext *thread_context, JobContext *job_context, const int thread_id) {
    *thread_context = *new ThreadContext(job_context, thread_id);
}
...
ThreadContext *init_thread_contexts(OutputVec &outputVec, int multiThreadLevel, const MapReduceClient &client,
    ...
    for (int thread_id = 0; thread_id < multiThreadLevel; ++thread_id) {
        init_thread_context(thread_contexts+thread_id,job_context,thread_id);
    }
...
startMapReduceJob(const MapReduceClient &client, const InputVec &inputVec, OutputVec &outputVec, int multiThreadLevel) {
    ...
    auto *job_context = new JobContext(job_state, threads, multiThreadLevel, outputVec, inputVec, client,
                                       multiThreadLevel);
